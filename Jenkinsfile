    ...
    stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh "docker run --name khoa-mysql --publish 3307:3306 --rm --network dev -v khoa-mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_LOGIN_PSW} -e MYSQL_DATABASE=library  -d mysql:8.0 "
                sh 'sleep 20'
